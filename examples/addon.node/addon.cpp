...
void whisper_print_segment_callback(struct whisper_context * ctx, struct whisper_state * state, int n_new, void * user_data) {
    ...
    const auto & segment_callback = *((whisper_print_user_data *) user_data)->segment_callback;
    ...
    for (int i = s0; i < n_segments; i++) {
        ...
        if (!params.no_timestamps) {
            ofprintf(stderr, "[%s --> %s]  ", to_timestamp(t0).c_str(), to_timestamp(t1).c_str());
        }
        ...
        if (params.diarize && pcmf32s.size() == 2) {
            ...
            //ofprintf(stderr, "is0 = %lld, is1 = %lld, energy0 = %f, energy1 = %f, %s\n", is0, is1, energy0, energy1, speaker.c_str());
        }
        ...
        segment_callback.BlockingCall([text, t0, t1](Napi::Env env, Napi::Function jsCallback) {
            ...
            result.Set(Napi::String::New(env, "from"), Napi::Number::New(env, t0));
            result.Set(Napi::String::New(env, "to"), Napi::Number::New(env, t1));
            result.Set(Napi::String::New(env, "text"), Napi::String::New(env, text));
...
int run(whisper_params &params, std::vector<std::vector<std::string>> &result, Napi::ThreadSafeFunction& segment_callback, Napi::ThreadSafeFunction& progress_callback) {
    ...
    if (params.language != "auto" && whisper_lang_id(params.language.c_str()) == -1) {
        ofprintf(stderr, "error: unknown language '%s'\n", params.language.c_str());
        exit(0);
    }
    ...
    for (int f = 0; f < (int) params.fname_inp.size(); ++f) {
        ...
        if (!::read_wav(fname_inp, pcmf32, pcmf32s, params.diarize)) {
            ofprintf(stderr, "error: failed to read WAV file '%s'\n", fname_inp.c_str());
            continue;
        }
        ...
        {
            ofprintf(stderr, "\n");
            ofprintf(stderr, "system_info: n_threads = %d / %d | %s\n",
        ...
        // print some info about the processing
        {
            ...
            if (!whisper_is_multilingual(ctx)) {
                if (params.language != "en" || params.translate) {
                    ...
                    ofprintf(stderr, "%s: WARNING: model is not multilingual, ignoring language and translation options\n", __func__);
                }
            }
            ofprintf(stderr, "%s: processing '%s' (%d samples, %.1f sec), %d threads, %d processors, lang = %s, task = %s, timestamps = %d ...\n",
        ...
        // run the inference
        {
            ...
            if (whisper_full_parallel(ctx, wparams, pcmf32.data(), pcmf32.size(), params.n_processors) != 0) {
                ofprintf(stderr, "failed to process audio\n");
                return 10;
            }
        }
    }
